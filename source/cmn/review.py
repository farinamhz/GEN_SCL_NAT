...
class Review(object):
    ...
    def translate_batch(reviews, tgt, settings):
        ...
        if settings['translator'] == 'nllb':
            ...
            # making batch translation faster by joining the reviews into one file
            grouped_strings = []
            grouped_string = ''
            for index, string in enumerate(reviews_txt, 1):
                grouped_string += string + '\n\n'
                if index % 10 == 0 or index == len(reviews_txt):
                    grouped_strings.append(grouped_string.strip())
                    grouped_string = ''
            translated_batch = GoogleTranslator(source=src, target=tgt).translate_batch(grouped_strings)
            translated_txt = []
            for group in translated_batch:
                translated_txt.extend(group.split('\n\n'))

            back_grouped_strings = []
            back_grouped_string = ''
            for index, string in enumerate(translated_txt, 1):
                back_grouped_string += string + '\n\n'
                if index % 10 == 0 or index == len(translated_txt):
                    back_grouped_strings.append(back_grouped_string.strip())
                    back_grouped_string = ''
            back_translated_batch = GoogleTranslator(source=tgt, target=src).translate_batch(back_grouped_strings)
            back_translated_txt = []
            for group in back_translated_batch:
                back_translated_txt.extend(group.split('\n\n'))
